...
export default function DefaultPage() {
  ...
  useEffect(() => {
    if (!(location.pathname === "/new")) {
      ...
      dispatch(fetchTabsData(decodeURIComponent(location.pathname.slice(1)))).then(res=>{
    ...
    <>
      ...
      >
        <div className="flex flex-col gap-5 items-center h-screen">
          {!(location.pathname === "/new") && <div className="flex items-start w-full md:w-4/5">
            <h1 className="font-bold text-3xl text-left self-center mr-2">{(!title && location.pathname ==="/new") ? title : (decodeURIComponent(location.pathname.slice(1))) }</h1>
              <button
                onClick={handleSave}
          ...
          <div className="flex flex-wrap gap-5 text-lg w-full md:w-4/5">
            {tabs?.data?.tabsList?.map((_, idx) => (
              ...
              <button onClick={()=>handleDeleteTab(idx)} className="absolute m-0 p-0 w-6 h-6 flex justify-center items-center -right-2 -top-2 bg-black text-white rounded-full"><span className="mb-1" style={{width: "-webkit-fill-available", borderRadius: "100%"}}><b>&times;</b></span></button>
            </div>
            ))}
            <button
